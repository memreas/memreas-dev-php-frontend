...
class IndexController extends AbstractActionController
{
    ...
    private function handleWSSession ($action, $result)
    {
        ...
        if ($action == 'login') {
            ...
            // error_log ( 'handleWSSession.user_id--->' . print_r ( $_SESSION,
        // true ) . PHP_EOL );
        } else 
            if ($action == 'logout') {
                /**
    ...
    private function getS3Key ()
    {
        $this->memreas_session();
        $action = 'memreas_tvm';
        $xml = '<xml><username>' . $_SESSION['username'] .
                 '</username><memreas_tvm>1</memreas_tvm></xml>';
    ...
    private function fetchMemreasTVMPreSignedURLAction ()
    {
        ...
        $action = 'memreas_tvm';
        $xml = '<xml><username>' . $_SESSION['username'] .
                 '</username><memreas_tvm>0</memreas_tvm><memreas_pre_signed_url>1</memreas_pre_signed_url></xml>';
        $s3Authenticate = $this->fetchXML($action, $xml);
        return $s3Authenticate;
    }
    ...
    private function generateMediaIdAction ()
    {
        ...
        $xml = '<xml><username>' . $_SESSION['username'] .
                 '</username><generatemediaid></generatemediaid></xml>';
        $s3Authenticate = $this->fetchXML($action, $xml);
        return $s3Authenticate;
    }
    ...
    public function writeJsConstants ()
    {
        ...
        $fileHandle = fopen(
                $_SERVER['DOCUMENT_ROOT'] . '/memreas/js/constants.js', 'w');
    ...
    public function editmediaAction ()
    {
        ...
        $s3Object = new S3(MemreasConstants::S3_APPKEY, 
                MemreasConstants::S3_APPSEC);
        ...
        $s3Object->putBucket(MemreasConstants::S3BUCKET, 
                $s3Object::ACL_PUBLIC_READ_WRITE);
        
        $s3Object->putObjectFile($server_source_file, 
                MemreasConstants::S3BUCKET, $target_file, 
                $s3Object::ACL_PUBLIC_READ_WRITE, array(), 'image/jpeg');
    ...
    public function audiocommentAction ()
    {
        ...
        if (isset($_POST['file_data'])) {
            ...
            $target_dir = getcwd() . '/app/memreas/user_comment_files/' .
                     $user_id . '/';
            ...
            $result = $s3Object->putObjectFile($server_source_file, 
                    MemreasConstants::S3BUCKET, $target_file, 
                    $s3Object::ACL_PUBLIC_READ_WRITE, array(), 'audio/wav');
            ...
            if ($data->addmediaeventresponse->status == 'Success') {
                ...
                $xml_add_comment .= '<audio_media_id>' . $audio_comment_id .
                         '</audio_media_id >';
                $xml_add_comment .= '</addcomment>';
                $xml_add_comment .= '</xml>';
    ...
    public function downloadMediaAction ()
    {
        ...
        $requestUrl = str_replace("file=", "", $requestUrl[1]) . '?' .
                 $requestUrl[2];
    ...
    public function fetchUserIPAddress ()
    {
         ...
         */
        $this->ipAddress = $this->getServiceLocator()
            ->get('Request')
