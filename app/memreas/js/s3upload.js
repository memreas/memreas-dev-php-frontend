...
/*
 * S3 System class and handling site media uploading functionally @param Element
 * Target form class/id name will be doomed
 */
var FileInstances = [];
	...
	this.doomMediaUploadForm = function(element) {
		...
		self.instance
				.each(function() {
					...
					//
					// deep copy data to be upload
					//
					var uploadHolder = $('.upload-from-event');
					var upload = jQuery.extend(true, {}, uploadHolder);
					form
							.fileupload({
								dropZone : upload,
								...
								add : function(event, data) {
									...
									// Will switch to target handling snippet
									// code for each form type
																...
																if (getValueFromXMLTag(
																	...
																	form
																			.find(
																					'input[name=key]')
																			.val(
																					FileKeyInstances[filename]);
																	FileInstances[filename]
																			.submit();
					...
					form
							.fileupload({
								...
								add : function(event, data) {
									...
									// fallback to
									// iframe
									options = $(form).fileupload('option');
									use_xhr = !options.forceIframeTransport
											&& ((!options.multipart && $.support.xhrFileUpload) || $.support.xhrFormDataFileUpload);
					...
					form
							.fileupload({
								...
								success : function(data, status, jqXHR) {
									...
									var _media_url = getValueFromXMLTag(
											jqXHR.responseText, 'Key');
									var _media_extension = _media_url
											.split(".");
