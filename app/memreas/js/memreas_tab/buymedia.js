					...
					.ajax({
						...
						success : function(response) {
							...
							if (response.status == 'Success') {
								...
								// tab is activated
								var sell_class = '';
								if ($("a[title=memreas-tab3]").parent('li')
										.attr("id") == "current")
									sell_class = "public-";
								else
									sell_class = "private-";

								var jElement = $("#" + sell_class + "selling-"
										+ current_event);
								var creator_id = jElement.attr("data-owner");
								var divaTagClick = "showEventDetail('"
										+ current_event + "', '" + creator_id
										+ "');";
								var aTagClick = "javascript:showEventDetail('"
										+ current_event + "', '" + creator_id
										+ "');";
								jElement.removeAttr("data-owner").removeAttr(
										"data-click").find("a").attr("href",
										aTagClick);
