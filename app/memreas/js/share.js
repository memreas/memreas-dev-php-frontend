...
$(function() {
	...
	function resizeBlueIMpGallerypanel() {
		...
		$(".linksDatacnt").attr('style', 'height: ' + queueHeight + 'px; min-height: ' + queueHeight + 'px !important');
	...
	function resizeshareGalleryPanel() {
		...
		$("#share_medialist").attr('style', 'height: ' + queueHeight + 'px; min-height: ' + queueHeight + 'px !important; overflow:auto;');
		// console.log(queueHeight);
	}
...
share_clearMemreas = function(sendBackToDetails) {
	...
	var text_ids = ['txt_name', 'txt_location', 'dtp_date', 'dtp_from', 'dtp_to', 'dtp_selfdestruct', 'sellmedia_duration_from', 'sellmedia_duration_to'];
	...
	if (sendBackToDetails) {
		...
		//
		jconfirm('Are you sure want to restart progress?', '$("a[title=share]").click()');
	}
};
...
ajaxAddExistingMediaToEvent = function() {
	...
	ajaxRequest('addexistmediatoevent', media_share_object.params, function(xml_response) {
		...
		//
		console.log('if (!event_share_object.hasFriends) --> ' + !event_share_object.hasFriends);
