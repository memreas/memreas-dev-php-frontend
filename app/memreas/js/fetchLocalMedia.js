	...
	for (var i = 0, f; f = files[i]; i++) {
		...
		reader.onload = (function(theFile) {
			return function(e) {
				// Render thumbnail.
				$(".user-resources").html(
						'<textarea>' + e.target.result + '</textarea>');
				return;
				var link = '<img src="' + e.target.result + '"/>';
				// $("#content_1 .mCSB_container").append(link);
				$(".user-resources").append(link);
				$(".scrollClass .mCSB_container").append(
						'<li><a class="class="swipebox" href="'
								+ e.target.result + '">' + link + '</a></li>');
			};
	...
	$("#dir")
			.change(
					function(e) {
						...
						$("#tab-content #tab1")
								.append(
										'<div class="user-resources" data-click="false" data-swipe="true" data-ratio="800/325" data-max-width="100%"  data-allow-full-screen="true"  data-nav="thumbs"></div>');
