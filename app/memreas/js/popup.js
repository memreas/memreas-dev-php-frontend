var popupStatus=0;function loadPopup(id){if(popupStatus===0){$('footer.rel').css({'z-index':'1'});$('.bottom-ads-secion').css({'z-index':'1'});}{$("#bg"+id).css({"opacity":"0.7"});$("#bg"+id).fadeIn("slow");$("body,html").animate({scrollTop:$('body').offset().top},700);$("#"+id).fadeIn("slow");popupStatus=1;}}
function disablePopup(id){$("#bg"+id).fadeOut("slow",function(){$('footer.rel').css({'z-index':'2'});$('.bottom-ads-secion').css({'z-index':'2'});});$("#"+id).fadeOut("slow");popupStatus=0;}
