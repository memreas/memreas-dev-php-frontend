...
function uploadAudio(blobObject, tabActive) {
    ...
    if (tabActive == 'share'){
        file2.onloadend = function (e) {
            $.ajax({url: "/index/audiocomment", type: "POST", data: {file_data: file2.result, user_id: $('input[name=user_id]').val(), comment_event_id: event_id}, dataType: "html", success: function (response) {
                $('#loadingpopup').hide();
                jsuccess(response);
            }});
        };
        ...
        file2.onloadend = function (e) {
            $.ajax({url: "/index/audiocomment", type: "POST", data: {file_data: file2.result, user_id: $('input[name=user_id]').val(), comment_event_id: eventdetail_id, media_id:eventdetail_media_id}, dataType: "html", success: function (response) {
                $('#loadingpopup').hide();
                jsuccess(response);
            }});
        };
