...
//console.log('event_creator_user_id-->' + JSON.stringify(friends, null, '\t'));
console.log('event_creator_user_id-->' + creator_id);
                        if (friendMemreasType == 'private'){
                        ...
                        if (typeof ($(friend).filter('profile_pic_79x80')) != 'undefined'){
                            var profile_img = $(friend).filter('profile_pic_79x80').html();
...
console.log('$(friend).filter("profile_pic_79x80").html()-->' + removeCdataCorrectLink($(friend).filter('profile_pic_79x80').html()));
console.log('$(friend).filter("profile_pic_448x306").html()-->' + removeCdataCorrectLink($(friend).filter('profile_pic_448x306').html()));
console.log('$(friend).filter("profile_pic_98x78").html()-->' + removeCdataCorrectLink($(friend).filter('profile_pic_98x78').html()));
                    	 ...
                    	 */
                        var events_resources = $(friend).filter ('events').html();
                        var event_resources = $(events_resources).filter ('event');
                        var event_resources_count = event_resources.length;
                        var total_event_row_width = 120 * event_resources_count;
                        
console.log('event_resources_count-->' + event_resources_count);
...
console.log('event_resource-->' + event_resource);
                                var eventId = $(event_resource).filter('event_id').html();
console.log('eventId-->' + eventId);
								//fetch event media 
								var event_media_resource = $(event_resource).filter ('event_media').html();

                                var event_media_98x78 = $(event_media_resource).filter('event_media_98x78').html();
console.log('event_media_98x78-->' + event_media_98x78);
								event_media_98x78 = removeCdataCorrectLink(event_media_98x78);
console.log('event_media_98x78 after-->' + event_media_98x78);
                                if (event_media_98x78 == '') event_media_98x78 = '/memreas/img/small/1.jpg';
                                var event_name = $(event_resource).filter ('event_name').html();
                                var event_metadata = $(event_resource).filter ('event_metadata').html();
console.log('event_name-->' + event_name);
console.log('event_metadata-->' + event_metadata);
...
console.log('sell_price-->' + sell_price);
                                    $("#" + friend_row).append ('<div class="event_img"><a href="javascript:showEventDetail(\'' + eventId + '\', \'' + creator_id + '\');">' + '<img src="' + event_media_98x78 + '" alt="">' + '</a>'+'<span class="event_name_box"><a style="color:#FFF;" href="javascript:showEventDetail(\'' + eventId + '\', \'' + creator_id + '\');">!' + event_name + '</a></span></div>');
                                } else {
console.log('selling event???--');
                                    $("#" + friend_row).append ('<div class="event_img" id="' + sell_class + 'selling-' + eventId + '" data-owner="' + creator_id + '" data-click="popupBuyMedia(\'' + eventId + '\', \'' + sell_price + '\', \'' + event_name  + '\');"><div class="sell-event-overlay"></div><span class="sell-event-buyme"><i>checking...</i></span><img src="' + event_media_98x78 + '" alt=""><span class="event_name_box"><a style="color:#FFF;" href="javascript:;">!' + event_name + '</a></span></div>');
                            ...
                            } // end for loop for (var key=0;key < friend_resources_count;key++)
