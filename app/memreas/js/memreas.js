			...
			function(response) {
				if (getValueFromXMLTag(response, 'status') == "Success") {
					...
					for (var i = 0; i < event_count; i++) {
						...
						/*
						 * This section isn't necessary ... view events passes
						 * back all this data media, friends, comments.
						 */
						/*
						 * // get event medias ajaxRequest( 'listallmedia', [ {
						 * tag : 'event_id', value : eventId }, { tag :
						 * 'user_id', value : user_id }, { tag : 'device_id',
						 * value : device_id }, { tag : 'limit', value :
						 * media_limit_count }, { tag : 'page', value :
						 * media_page_index } ], function(response) { var
						 * eventId = getValueFromXMLTag(response, 'event_id');
						 * var jMediaElement = $("#myEvent-" + eventId); if
						 * (getValueFromXMLTag(response, 'status') == "Success") {
						 * var medias = getSubXMLFromTag(response, 'media'); var
						 * html_media = ''; if (typeof (eventId != 'undefined')) {
						 * var media_count = medias.length; for (var i = 0; i <
						 * media_count; i++) { var media = medias[i]; var
						 * mediaid = $(media).find( 'media_id')[0].innerHTML;
						 * var media_type = $(media) .filter('type').html(); var
						 * _media_url = getMediaThumbnail( media,
						 * '/memreas/img/small/1.jpg'); if (media_type ==
						 * 'video') html_media += '<div id="emedia-' + mediaid + '"
						 * class="event_img video-media"><a href="javascript:;"
						 * eventid="' + eventId + '" data="' + mediaid + '"><img
						 * src="' + _media_url + '"/><img
						 * class="overlay-videoimg"
						 * src="/memreas/img/video-overlay.png" /></a></div>';
						 * else html_media += '<div id="emedia-' + mediaid + '"
						 * class="event_img"><a href="javascript:;" eventid="' +
						 * eventId + '" data="' + mediaid + '"><img src="' +
						 * _media_url + '"/></a></div>'; } if (html_media !=
						 * '') { jMediaElement.empty().html( html_media); if
						 * ($("#viewport .event_img").length) { $("#viewport
						 * .event_img a") .each( function() { $(this) .click(
						 * function() { $( ".modal-backdrop") .removeClass(
						 * "out") .addClass( "in"); $( ".modal-backdrop")
						 * .fadeIn(); $( ".modal-backdrop") .html( '<p class="loading" style="clear: both;width:100%;text-align:center;padding-top:20%"><img
						 * src="/memreas/img/loading_animate.gif"
						 * style="width:55px;"/></p>'); var
						 * eventdetail_media_id = $( this) .attr( "data"); var
						 * eventdetail_id = $( this) .attr( "eventid"); // get //
						 * media // comments ajaxRequest( 'listcomments', [ {
						 * tag : 'event_id', value : eventdetail_id }, { tag :
						 * 'media_id', value : eventdetail_media_id }, { tag :
						 * 'limit', value : '100' }, { tag : 'page', value : '1' } ],
						 * function( ret_xml) {
						 * 
						 * var event_comments = getSubXMLFromTag( ret_xml,
						 * 'comment'); var comment_count =
						 * event_comments.length; if (comment_count > 0) { var
						 * html_str = '<ul class="commitems">'; for (var i =
						 * 0; i < comment_count; i++) { var event_comment =
						 * event_comments[i]; var comment_owner_pic =
						 * getValueFromXMLTag( event_comment, 'profile_pic');
						 * comment_owner_pic =
						 * removeCdataCorrectLink(comment_owner_pic); if
						 * (comment_owner_pic == '') comment_owner_pic =
						 * '/memreas/img/profile-pic.jpg'; var comment_text =
						 * getValueFromXMLTag( event_comment, 'comment_text');
						 * var comment_type = getValueFromXMLTag( event_comment,
						 * 'type'); html_str += '<li>' + '<figure
						 * class="pro-pics"><img src="' + comment_owner_pic + '"
						 * alt=""></figure>';
						 *  // Comment // is // text // or // audio if
						 * (comment_type == 'text') html_str += '<textarea
						 * readonly="readonly">' + comment_text + '</textarea>';
						 * else { var audio_media_url = getValueFromXMLTag(
						 * event_comment, 'audio_media_url'); audio_media_url =
						 * removeCdataCorrectLink(audio_media_url); html_str += '<audio
						 * controls class="memreas-detail-audio">' + '<source
						 * src="' + audio_media_url + '" type="audio/wav" />' +
						 * 'Your browser does not support the audio element' + '</audio>'; }
						 * html_str += '</li>';
						 *  } html_str += '</ul><div style="clear:both"></div>';
						 * 
						 * popupDetailMedia( eventdetail_media_id, html_str,
						 * comment_count);
						 *  // // if( //
						 ...
						 * else { var html_str = '<p style="color: #FFF;" class="no-comment">No
						 * comment yet!</p>'; popupDetailMedia(
						 * eventdetail_media_id, html_str, 0); } }, 'undefined',
						 * true); }); }); } } else jMediaElement .empty() .html( '<div
						 * class="event_img video-media">There is no media on
						 * this event</div>'); } } else { jMediaElement
						 * .empty() .html( '<i style="color: #FFFFFF;">There is
						 * no media on this event</i>'); } }, 'undefined',
						 * true);
						 *  // Get event people ajaxRequest( 'geteventpeople', [ {
						 * tag : 'event_id', value : eventId } ],
						 * function(xml_response) { var html_people = ''; var
						 * response_event_id = getValueFromXMLTag( xml_response,
						 * 'event_id'); var jEvent_people = $("#event-people-" +
						 * response_event_id); var friends = getSubXMLFromTag(
						 * xml_response, 'friend'); if
						 * (getValueFromXMLTag(xml_response, 'status') ==
						 * 'Success') { var count_people = friends.length; for
						 * (i = 0; i < count_people; i++) { var friend =
						 * friends[i]; var friend_photo = ''; if
						 * (getValueFromXMLTag(friend, 'photo') == '' ||
						 * getValueFromXMLTag( friend, 'photo') == 'null')
						 * friend_photo = '/memreas/img/profile-pic.jpg'; else
						 ...
						 * 'friend_name'); html_people += '<div
						 * class="event_gallery_pro"><img src="' + friend_photo + '"
						 * alt="' + friend_name + '" title="' + friend_name + '" /></div>'; }
						 * jEvent_people.html(html_people); } else
						 * jEvent_people.html(''); }, 'undefined', true);
						 * $("#myEvent-" + eventId).swipe({ TYPE : 'mouseSwipe',
						 * HORIZ : true }); $("#swipebox-comment-" +
						 * eventId).swipe({ TYPE : 'mouseSwipe', HORIZ : true
						 * });
						 */
					}
				} else
